...
我们先从寻求任一满足初值条件$\varphi(0)=\pmb{\eta}$的解$\varphi(t)$开始。

从定理9我们知道，$\varphi(t)$可以表为$\varphi(t)=(\exp At)\pmb{\eta}$，而我们的目标就是要将$(\exp At)\pmb{\eta}$明显地计算出来，即要确切知道$\varphi(t)$的每一个分量。根据$\exp At$的定义，一般来说，$(\exp At)\pmb{\eta}$的分量是一个无穷级数，因而难于计算。这里的要点就是将初始向量$\pmb{\eta}$进行分解，从而使得$(\exp At)\pmb{\eta}$的分量可以表示为$t$的指数函数与$t$的幂函数乘积的有限项的线性组合。

假设$\lambda_1,\lambda_2,\cdots,\lambda_k$分别是矩阵$A$的$n_1,n_2,\cdots,n_k$重不同特征值。这时我们有
$$
\begin{align}
\pmb{\eta}=v_1+v_2+\cdots+v_k\tag{5.50}
\end{align}
$$
其中$v_i\in U_j(j=1,2,\cdots,k)$是$A$的一组线性无关的特征向量，因为子空间$U_j$是由方程组$(A-\lambda_jE)^{n_j}u=0$产生的，$v_j$一定是其的解。由此即得
$$
\begin{align}
(A-\lambda_jE)^{l}v_j=0,\quad l\geqslant n_j,\quad j=1,2,\cdots,k\tag{5.51}
\end{align}
$$
注意到当矩阵是对角矩阵时，$\exp At$是很容易求得的，这时得到
$$
e^{\lambda_jt}\exp(-\lambda_jEt)=e^{\lambda_jt}
\begin{bmatrix}
e^{-\lambda_jt} & & & \\
 & e^{-\lambda_jt} & & \\
 & & \ddots & \\
...
$$
\begin{align}
(\exp At)v_j&=(\exp At)e^{\lambda_jt}[\exp(-\lambda_jEt)]v_j \\
&=e^{\lambda_jt}[\exp(A-\lambda_jE)t]v_j \\
&=e^{\lambda_jt}\left[E+t(A-\lambda_jE)+\displaystyle\frac{t^2}{2!}(A-\lambda_jE)^2+\cdots+\displaystyle\frac{t^{n_j-1}}{(n_j-1)!}(A-\lambda_jE)^{n_j-1}\right]v_j.
\end{align}
$$
再根据等式(5.50)，知微分方程组(5.33)的解$\varphi(t)=(\exp At)\pmb{\eta}$可表示为
$$
\begin{align}
\varphi(t)&=(\exp At)\pmb{\eta}=(\exp At)\displaystyle\sum_{j=1}^{k}v_j=\displaystyle\sum_{j=1}^{k}(\exp At)v_j \\
&=\displaystyle\sum_{j=1}^{k}e^{\lambda_jt}\left[E+t(A-\lambda_jE)+\displaystyle\frac{t^2}{2!}(A-\lambda_jE)^2+\cdots+\displaystyle\frac{t^{n_j-1}}{(n_j-1)!}(A-\lambda_jE)^{n_j-1}\right]v_j,
\end{align}
$$
所以，方程(5.33)满足$\varphi(0)=\pmb{\eta}$的解$\varphi(t)$最后可以写成
$$
\begin{align}
\varphi(t)=\displaystyle\sum_{j=1}^{k}e^{\lambda_jt}\left[\displaystyle\sum_{i=0}^{n_j-1}\displaystyle\frac{t^i}{i!}(A-\lambda_jE)^i\right]v_j\tag{5.52}
\end{align}
$$
在特别情形，当$A$只有一个特征值时，无需将初始向量分解为(5.50)。这时对于任何$u$，都有
$$
(A-\lambda E)^nu=0,
$$
这就是说，$(A-\lambda E)^n$是一个零矩阵，这样一来，由$\exp At$的定义，我们得到
$$
\begin{align}
\exp At&=e^{\lambda t}\exp(A-\lambda E)t \\
&=e^{\lambda t}\displaystyle\sum_{i=0}^{n-1}\displaystyle\frac{t^i}{i!}(A-\lambda E)^i.\tag{5.53}
\end{align}
$$
为了要(5.52)从中得到$\exp At$，只要注意到
$$
\exp At=(\exp At)E=[(\exp At)e_1,(\exp At)e_2,\cdots,(\exp At)e_n],
$$
其中
$$
e_1=
...
\end{bmatrix},
$$
是单位向量。这就是说，依次令$\pmb{\eta}=e_1,\pmb{\eta}=e_2,\cdots,\pmb{\eta}=e_n$，求得$n$个解，以这$n$个解作为列即可得到$\exp At$。
...
$$
那么，<br>
1.如果$A$的特征值的实部都是负的，则(5.33)的任一解当$t\to+ \infty$时都趋于零；<br>
2.如果$A$的特征值的实部都是非正的，且实部为零的特征值都是简单特征值，则(5.33)的任一解当$t\to+ \infty$时都保持有界；<br>
3.如果$A$的特征值至少有一个具有正实部，则(5.33)至少有一个解当$t\to+ \infty$时趋于无穷。

附注2 <br>
利用若尔当标准形计算基解矩阵。

首先对于矩阵$A$，由矩阵理论知道，必存在非奇异的矩阵$T$，使得
$$
\begin{align}
T^{-1}AT=J\tag{5.55}
\end{align}
...
\lambda_j & 1 & & & \\
 ...
 & & & \ddots & 1 \\
 & & & & \lambda_j
\end{bmatrix}(j=1,2,\cdots,l)
$$
为$n_j$阶矩阵，并且$n_1+n_2+\cdots+n_l=n$，而$l$为矩阵$A-\lambda E$的初级因子的个数；$\lambda_1,\lambda_2,\cdots,\lambda_l$是特征方程$\det(\lambda E-A)\equiv0$的根，其间可能有相同者；矩阵中空白的元均为零。

由于矩阵$J$及$J_i(i=1,2,\cdots,l)$的特殊形式，容易计算得到
$$
\begin{align}
\exp Jt=
\begin{bmatrix}
...
\exp J_jt=
\begin{bmatrix}
1 & t & \displaystyle\frac{t^2}{2!} & \cdots & \displaystyle\frac{t^{n_{j-1}}}{(n_j-1)!} \\
 & 1 & t & \cdots & \displaystyle\frac{t^{n_{j-2}}}{(n_j-2)!} \\
 & & & \ddots & \vdots \\
 & & & & 1 \\
\end{bmatrix}\tag{5.57}
\end{align}
$$
所以，如果矩阵$J$是若尔当标准形，那么可以计算得到$\exp Jt$，由(5.55)及矩阵指数的性质，可以得到微分方程组(5.33)的基解矩阵$\exp At$的计算公式
$$
\begin{align}
\exp At=\exp(TJT^{-1})t=T(\exp Jt)t^{-1}\tag{5.58}
\end{align}
$$
当然，矩阵
$$
\begin{align}
\Psi(t)=T\exp Jt\tag{5.59}
\end{align}
$$
也是(5.33)的基解矩阵。
...
$$
\exp At=\displaystyle\sum_{j=0}^{n-1}r_{j+1}(t)P_j,
$$
其中$P_0=E,P_j=\displaystyle\prod_{k=1}^{j}(A-\lambda_kE)(j=1,2,\cdots,n)$，而$r_1(t),r_2(t),\cdots,r_n(t)$是初值问题
$$
\begin{cases}
r_1'=\lambda_1r_1,\\
r_j'=r_{j-1}+\lambda_jr_j(j=2,3,\cdots,n),\\
r_1(0)=1,r_j(0)=0(j=2,3,\cdots,n)
\end{cases}
$$
的解，$\lambda_1,\lambda_2,\cdots,\lambda_n$是矩阵$A$的特征值(不必相异)。

最后，我们给出非齐次线性微分方程组
$$
\begin{align}
\pmb{x}'=A\pmb{x}+\pmb{f}(t)\tag{5.60}
\end{align}
$$
的常数变易公式，这里$A$是$n\times n$常数矩阵，$\pmb{f}(t)$是已知的连续向量函数。<br>
因为对应的齐次线性微分方程组的基解矩阵为$\Phi(t)=\exp At$，所以，我们有
$$
\Phi^{-1}(s)=\exp(-sA),\Phi(t)\Phi^{-1}(s)=\exp[(t-s)A]
$$
若初值条件是$\varphi(t_0)=\pmb{\eta}$，则$\varphi_k(t)=\exp[(t-t_0)A]\pmb{\eta}$,(5.60)的解就是
$$
\begin{align}
\varphi(t)=\exp[(t-t_0)A]\pmb{\eta}+\displaystyle\int_{t_0}^{t}\exp[(t-s)A]\pmb{f}(s)\text{d}s\tag{5.61}
\end{align}
$$
...
我们定义
$$
\mathscr{L}[\pmb{f}(t)]=\displaystyle\int_{0}^{+ \infty}e^{-st}\pmb{f}(t)\text{d}t,
$$
其中$\pmb{f}(t)$是$n$维向量函数，要求它的每一个分量都存在拉普拉斯变换。
