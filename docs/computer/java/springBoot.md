...
访问http://localhost:8080/api/success，
可以看到html页面内容
...
HttpMessageConverters是从容器中确定，获取所有的HttpMessageConverter; <br>
自己给容器中添加HttpMessageConverter，只需要将自己的组件注册容器中(`@Bean 或 @Component`)
...
接管前通过http://localhost:8080/static.html
...
将login.html,success.html放在resources/templates文件夹下
...
创建项目时dependencies添加SQL -> MySQL Driver
